...
// 	{
// 		auto gltfStream = streamReader->GetInputStream(
// 			pathFile.string()); // Pass a UTF-8 encoded filename to GetInputString
...
// 			pathFile.string()); // Pass a UTF-8 encoded filename to GetInputString
// 		auto glbResourceReader =
// 			std::make_unique<GLBResourceReader>(std::move(streamReader), std::move(glbStream));
...
// 			"Command line argument path filename extension must be .gltf or .glb");
// 	}

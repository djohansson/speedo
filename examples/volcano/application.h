...
private:
		...
		const PipelineConfiguration<B>& pipelineConfig) const
	{
		...
		PipelineHandle<B> outPipeline = 0;
		...
		const std::vector<Model<B>*>& models)
	{
		...
		// {
			myGraphicsPipelineConfig = std::make_shared<PipelineConfiguration<B>>();
			*myGraphicsPipelineConfig = createPipelineConfig(myDevice, createRenderPass(myDevice, window),
				myDescriptorPool, myPipelineCache,
				myGraphicsPipelineLayout, myDefaultResources);
		//}
	...
	// todo: put these two in hashmaps
